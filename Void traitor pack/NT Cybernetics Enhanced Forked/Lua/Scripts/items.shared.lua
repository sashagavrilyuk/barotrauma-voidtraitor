...
local function evaluateExtraUseInHealthInterface()
	...
	for _, tool in ipairs(NTCyb.AllowInHealthInterface) do
		if ItemPrefab.Prefabs.ContainsKey(tool) then ItemPrefab.Prefabs[tool].set_UseInHealthInterface(true) end
	end
end
...
local function addNullableImmutableArrayElements(builder, array)
	...
	if success then
		for i = 0, length - 1 do
			if not builder.Contains(array[i]) then builder.Add(array[i]) end
		end
	end
end
	...
	for _, xml in ipairs(NTCyb.ExtraSkillRequirementHints) do
		...
		if ItemPrefab.Prefabs.ContainsKey(itemIdentifier) then
			...
			for element in xdoc.Root.Elements() do
				...
				if not skillReqsBuilder.Contains(skillReq) then skillReqsBuilder.Add(skillReq) end
			end
...
local function addCyberOrgansToSuperSoldiersTalent()
	if not TalentPrefab.TalentPrefabs.ContainsKey("supersoldiers") then return end
...
local function patchDamageWeldingTool(itemId)
	if not ItemPrefab.Prefabs.ContainsKey(itemId) then return end
	...
	if targetElement == nil then return end
