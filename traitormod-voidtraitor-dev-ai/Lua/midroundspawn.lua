...
m.SetSpawnedClient = function (client, character)
    hasBeenSpawned[Traitormod.GetClientAccountKey(client)] = character
end
...
m.SpawnClientCharacterOnSub = function(submarine, client)
    ...
    hasBeenSpawned[Traitormod.GetClientAccountKey(client)] = spawned
...
Hook.Add("roundStart", "Traitormod.MidRoundSpawn.RoundStart", function ()
    ...
    for key, client in pairs(Client.ClientList) do
        if not client.SpectateOnly then
            hasBeenSpawned[Traitormod.GetClientAccountKey(client)] = true
        else
...
Hook.Add("client.connected", "Traitormod.MidRoundSpawn.ClientConnected", function (newClient)
    ...
    if not Game.RoundStarted or hasBeenSpawned[Traitormod.GetClientAccountKey(newClient)] then return end
...
Hook.Add("think", "Traitormod.MidRoundSpawn.Think", function ()
    ...
    if Game.RoundStarted and checkTime and Timer.GetTime() > checkTime then
        ...
        for i = #newPlayers, 1, -1 do
            ...
            if newClient and not hasBeenSpawned[Traitormod.GetClientAccountKey(newClient)] and (giveSpectatorsSpawnOption or not newClient.SpectateOnly) and newClient.Connection and newClient.Connection.Status == 1 then
