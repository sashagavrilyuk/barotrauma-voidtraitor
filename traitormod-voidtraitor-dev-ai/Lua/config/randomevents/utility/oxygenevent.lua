---@class OxygenEventConfig
---@field Name string
---@field ChancePerMinute number
---@field Message string
---@field Affliction string

---@param config OxygenEventConfig
---@return table
local function CreateOxygenEvent(config)
    local event = {}
    ...
    event.ChancePerMinute = config.ChancePerMinute
    ...
    local function applyAffliction(character)
        if character == nil or character.CharacterHealth == nil or character.AnimController == nil then
            return
        end

        local prefab = AfflictionPrefab.Prefabs[config.Affliction]
        if prefab == nil then
            return
        end

        character.CharacterHealth.ApplyAffliction(character.AnimController.MainLimb, prefab.Instantiate(20))
    end
    ...
    event.Start = function()
        ...
        Traitormod.RoundEvents.SendEventMessage(Traitormod.Language[config.Message], "GameModeIcon.PVP", Color.Red)
